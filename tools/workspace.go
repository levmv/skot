...
func NewWorkspaceToolsWithProtection(root string, protection *ProtectedPathPolicy) ([]agent.Tool, string, error) {
	access, err := NewFilesystemAccess(root, ScopeWorkspace, protection)
	if err != nil {
		return nil, "", err
	}
...
func (workspace *workspace) tools() []agent.Tool {
	tools := []agent.Tool{
		workspace.tool(
			...
			`{"type":"object","properties":{"path":{"type":"string","description":"File path. Relative paths start at the workspace; any path resolving outside it requires machine scope."},"offset":{"type":"integer","minimum":1,"description":"One-based first line to return. Defaults to 1."},"limit":{"type":"integer","minimum":1,"maximum":2000,"default":200,"description":"Maximum lines to return."}},"required":["path"],"additionalProperties":false}`,
			parallelSafe,
			workspace.read,
		),
		workspace.tool(
		...
		workspace.tool(
			...
			`{"type":"object","properties":{"pattern":{"type":"string","description":"Regular expression to search for."},"path":{"type":"string","description":"File or directory path. Relative paths start at the workspace; any path resolving outside it requires machine scope. Defaults to the workspace."},"include":{"type":"string","description":"Optional file glob such as *.go or **/*_test.go."}},"required":["pattern"],"additionalProperties":false}`,
			parallelSafe,
			workspace.grep,
		),
		workspace.tool(
			...
			`{"type":"object","properties":{"pattern":{"type":"string","description":"Glob such as **/*.go."},"path":{"type":"string","description":"Directory path. Relative paths start at the workspace; any path resolving outside it requires machine scope. Defaults to the workspace."}},"required":["pattern"],"additionalProperties":false}`,
			parallelSafe,
			workspace.glob,
		),
		workspace.tool(
			...
			`{"type":"object","properties":{"path":{"type":"string","description":"File path. Relative paths start at the workspace; any path resolving outside it requires machine scope."},"old_text":{"type":"string","description":"Exact text that must occur exactly once."},"new_text":{"type":"string","description":"Replacement text; may be empty."}},"required":["path","old_text","new_text"],"additionalProperties":false}`,
			serialTool,
			workspace.edit,
		),
		workspace.tool(
